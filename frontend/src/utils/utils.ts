...
const _utils = {
  //千帆流式接口js调用demo
  $parseSteamData: (url: string, access_token: string, body: any, onMessage: any) => {
    body.stream = true
  ...
  $checkIfWeLink: () => {
    ...
    const result =
      pattern.test(userAgent) || pcWelinkPattern.test(userAgent) || weCodeIDEpattern.test(userAgent)
